...
namespace BuildMonitor.Core.Actors
{
	...
	public class ProfileActor : ReceiveActor, IWithUnboundedStash
	{
		...
		private readonly Dictionary<IActorRef, (int, Screen)> _screens =
			new Dictionary<IActorRef, (int, Screen)>();
		...
		private void Ready() {
			...
			Receive<AggregatorResponse<Contracts.Actors.Screen, string>>(PublishProfileData);
			Receive<Init>(msg => ReInitialize());
		}
		...
		private void CollectProfileData(SendProfile msg) {
			...
			var aggregator =
				Context.ActorOf(Props.Create(() => new Aggregator<Contracts.Actors.Screen, string>(actors, msg.ConnectionId)));
			aggregator.Tell(ScreenDataRequest.Instance);
		}

		private void PublishProfileData(AggregatorResponse<Contracts.Actors.Screen, string> response) {
